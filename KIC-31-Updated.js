  ...
  if (element) {
    let KIC_31 = {
      ...
      mainJs: function () {
                   ...
                   <div class="product_img">
                	...
                	<div class="product_details">
                      ...
                      <div class="gallery_img_wrap">
                          ...
                          <div class="background-layer_two"></div>
                          <div class="background-layer_three"></div>
                      </div>
                      <div class="btn">
		...
		function getRandomIds(array) {
			const shuffledArray = array.sort(() => 0.5 - Math.random());
			return shuffledArray.slice(0,8)
		}
		const selectedIds = getRandomIds(products);
		
		const last_product_tile = document.querySelector('.KIC_31 .b-product_tile[data-widget="productTile"]:last-of-type');
		last_product_tile.insertAdjacentHTML("afterend",injectProduct(selectedIds));
		
		const existing_product_length = document.querySelectorAll('.b-product_tile').length;
		const l_plp_order = document.querySelectorAll('.b-product_tile').length+1;
	
		function productOrder(l_plp_grid,custom_products_one,custom_products_two,custom_products_three) {
		    document.querySelector('.l-plp_grid-actions').style.order = l_plp_grid;
		    document.querySelectorAll('.b-product_tile').forEach((item,i)=>{
		        item.style.order=i;
		    })
		    if(existing_product_length>custom_products_one){
		    	document.querySelectorAll('.products_wrapper')[0].style.order = custom_products_one;
		    }
		    if(existing_product_length>custom_products_two){
		    	document.querySelectorAll('.products_wrapper')[1].style.order = custom_products_two;
		    }
		    if(existing_product_length>custom_products_three){
		    	document.querySelectorAll('.products_wrapper')[2].style.order = custom_products_three;
		    }
		    document.querySelectorAll('.products_wrapper').forEach((product)=>{
		        if(!product.style.order) {
    	...
    	if(width >= 1500){
        	productOrder(l_plp_order,9,40);
    	}
    	else if(width >= 1000 && width <= 1499){
        	productOrder(l_plp_order,11,31);  
    	}
    	else if(width >= 769 && width <= 999){
        	productOrder(l_plp_order,8,28,46);
    	}
    	else if(width <= 768){
        	productOrder(l_plp_order,9,33);
    	}
    }
...
.KIC_31 .product_details a:hover {
    text-decoration: none;
}
